...
export function InteractiveProductCard({
  ...
  const [isLiked, setIsLiked] = React.useState(initialInWishlist);
  ...
  const handleLike = async (e: React.MouseEvent) => {
    e.stopPropagation();
    // Without a productId the heart is decorative (no persistence target).
    if (!productId) {
      setIsLiked((v) => !v);
      return;
    }
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        'relative aspect-[9/12] w-full max-w-[340px] cursor-pointer overflow-hidden rounded-2xl bg-[#1E1E1E] shadow-lg',
        'transform-style-3d',
        className
      )}
      {...props}
    >
      ...
      <div className="absolute inset-0 flex flex-col p-5" style={{ transform: 'translateZ(40px)' }}>
        {/* Header con título */}
        ...
        <div className="flex items-center justify-between rounded-xl border border-white/10 bg-black/40 p-3 backdrop-blur-md">
          <span className="text-lg font-bold text-[#FFCB05]">{price}</span>

          <div className="flex items-center gap-2">
            <button
              onClick={handleLike}
              disabled={likePending}
              aria-label={isLiked ? 'Eliminar de favoritos' : 'Añadir a favoritos'}
              aria-pressed={isLiked}
              className="rounded-full bg-white/10 p-2 transition-all hover:bg-white/20 disabled:opacity-60"
            >
              <Heart
                className={cn(
