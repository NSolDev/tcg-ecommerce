...
export default function SocialCards({ cards, autoplay = true, interval = 5000 }: SocialCardsProps) {
  ...
  return (
    ...
    >
              ...
              <div className="fan-carousel-card-image">
                ...
                {(card.title || card.description) && (
                  <div className={`fan-carousel-card-content ${isActive ? 'visible' : ''}`}>
                    {card.title && <h3 className="fan-carousel-card-title">{card.title}</h3>}
                    {card.description && (
                      <p className="fan-carousel-card-description">{card.description}</p>
                    )}
                  </div>
                )}
      ...
      {totalCards > 1 && (
        <>
          <button onClick={prevSlide} className="fan-carousel-control fan-carousel-control-left">
            <ChevronLeft className="fan-carousel-control-icon" />
          </button>
          <button onClick={nextSlide} className="fan-carousel-control fan-carousel-control-right">
        ...
        <div className="fan-carousel-indicators">
          {cards.map((_, index) => (
            <button
              ...
              className={`fan-carousel-indicator ${index === currentIndex ? 'active' : 'inactive'}`}
            />
          ))}
        </div>
      )}
